...
def ff(v1,v2,p1,p2,n):  #t1<t2
    ...
    # CASE2-4: FAST-SLOW & FAST-ZERO [t2]
    # if n < (p2/v2)*v1 - p1 + 1/2*(p2/v2)*v2 + p2/2
    # if n > (p2/v2)*v1 - p1
    if 3*p1*v2 <p2*v1 and v2*(2*n+2*p1-p2)<p2*(2*v1+v2) and v2*(n+p1)>p2*v1:
        return p2/v2
    ...
    # CASE2-5: SLOW-SLOW & SLOW-ZERO [t2]
    if 3*p1*v2 >=p2*v1 and v2*(2*n-p1-p2) < p2*(v1+v2) and v2*(2*n-p1)>p2*v1:
        return p2/v2
