...
const int N = 40004; vector<int> e[N]; bool match[N]; ll mt[N]; int sta[N],top;
...
bool solve(){
  ...
  for(int i=1;i<nm;++i){
    if(i%m==0) continue;
    x=mt[i];   ii=i<<1; y=mt[i-1]; jj=(i-1)<<1;
    if(y==x+1){e[ii^1].push_back(jj^1);e[jj].push_back(ii);} 
    if(x==y+1){e[jj^1].push_back(ii^1);e[ii].push_back(jj);} 
    if(y==x)  {e[jj].push_back(ii^1);e[jj^1].push_back(ii);e[ii].push_back(jj^1);e[ii^1].push_back(jj);}
  }
  for(int i=m;i<nm;++i){
    x=mt[i];   ii=i<<1; y=mt[i-m]; jj=(i-m)<<1;
    if(y==x+1){e[ii^1].push_back(jj^1);e[jj].push_back(ii);}
    if(x==y+1){e[jj^1].push_back(ii^1);e[ii].push_back(jj);} 
    if(y==x)  {e[jj].push_back(ii^1);e[jj^1].push_back(ii);e[ii].push_back(jj^1);e[ii^1].push_back(jj);}
  }
